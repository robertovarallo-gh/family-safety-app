...
const FamilyTrackingApp = () => {
  ...
  // AGREGA ESTOS CONSOLE.LOGS AQU脥:
  console.log('=== DEBUG ===');
...
const loadSafeZones = async () => {
  ...
  const handleAddChild = async (e) => {
    ...
    try {
      ...
      if (calculatedAge > 25) {
        throw new Error('La edad no puede ser mayor a 25 a帽os');
      }
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            ...
            role: calculatedAge < 13 ? 'ni帽o' : calculatedAge < 18 ? 'adolescente' : 'adulto'
          }
        }
      });
...
馃攼 CREDENCIALES DE ACCESO:
馃摟 Email: ${newChild.email}
馃攽 Password: ${temporaryPassword}

Para testing desde celular:
1. Abre: https://family-safety-app.vercel.app
2. Usa estas credenciales para login
3. El usuario debe cambiar su contrase帽a despu茅s del primer login`);
    ...
    <div className="max-w-md mx-auto bg-white min-h-screen">
      ...
      <div className="p-6">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <h3 className="font-semibold text-blue-800 mb-2">?? Proceso de invitación:</h3>
          <ul className="text-sm text-blue-700 space-y-1">
            <li>? Se enviará un email de invitación al miembro</li>
            <li>? Recibirá una contrase?a temporal para crear su cuenta</li>
            <li>? Podrá cambiar su contrase?a en el primer acceso</li>
            <li>? Una vez registrado aparecerá en tu panel familiar</li>
          </ul>
        </div>
        ...
        <form className="space-y-4">
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Se usará para calcular la edad automáticamente
            </p>
          </div>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="text-sm text-gray-600 space-y-2">
            <p><strong>1.</strong> El miembro recibirá un email con un enlace de registro</p>
            <p><strong>2.</strong> Creará su cuenta con una contrase?a personal</p>
            <p><strong>3.</strong> Una vez registrado aparecerá en tu panel familiar</p>
            <p><strong>4.</strong> Podrás ver su ubicación y comunicarte con él</p>
          </div>
        </div>
      </div>
    </div>
